    ...
    public void RefreshList()
    {
        ...
        foreach (Reservation res in BookingManager.Instance.allReservations)
        {
            ...
            if (res.isPlayerBooking)
            {
                ...
                Transform peopleTextObj = newEntry.transform.Find("PeopleText"); 

                // Updated: Added \n instead of | to put the date on the next line
                if (roomTextObj != null) roomTextObj.GetComponent<TextMeshProUGUI>().text = $"{res.roomName}\n{res.date}";
                ...
                // Set the amount of people text
                if (peopleTextObj != null) peopleTextObj.GetComponent<TextMeshProUGUI>().text = $"{res.amountOfPeople} personen";
